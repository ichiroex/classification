...
trainfile = open('train_movie.txt','r') #train file
...
testfile = open('test_movie.txt','r') #test file
num_inputsentence=0
num_right=0
for str in testfile:
  str = str.rstrip('\n')
  ...
  for word in array:
    ...
    if q_positive != 0 and q_negative != 0:
  ...
  if probpositive_test > probnegative_test:
    if rightclass == '+1':
      num_right+=1
  elif probnegative_test > probpositive_test:
    if rightclass == '-1':
      num_right+=1
